...
specijalni tip dvostruko povezane liste tzv XOR povezanu listu. Glavna razlika je
to sto je u ovom slucaju potreban samo jedan pointer po Node-u, a prethodnike i 
sljedbenike tog Node-a dobijamo XOR operacijom trenutnog pointera. Funkcijalnost
je u konacnici ista, medjutim ovaj oblik liste koristi duplo manje memorije jer
cuva samo jedan pointer (umjesto dva).
*/
...
private:
    struct Node
    {
        ...
        /* static Node *traverse(Node *curr, Node *prev) - Ovaj metod se koristi za kretanje naprijed ili nazad 
        kroz listu u zavisnosti  od redoslijeda proslijedjenih argumenata*/
        static Node *traverse(Node *curr, Node *prev)
        {
            ...
            Node *n = intToPtr(ptrToInt(prev) ^ curr->link_);
            if (n)
            {
                return n;
            }
        ...
        // Node *connect(Node *newNode) - Kreira vezu izmedju trenutnog (this) Node-a i  proslijedjenog Node-a
        ...
        /*
        void disconnectNodes(Node *a, Node *b, Node *c, Node *d) - Prekida vezu dva Node-a od ostatka liste.
        b i c su pointeri na Node-ove koje zelimo da otpojimo od ostatka liste. Koristi se konkretno za prekid veze
        head i tail Node-a prilikom brisanja (destructor). Potrebno je da prvo otpojimo Node-ove prije nego sto ih 
        obrisemo jer bez prethodnog Node-a nemoguce je znati adresu sljedeceg Node-a i obratno.
        */
    ...
    public:
        ...
        iterator_(const iterator_ &another) : prev_{another.prev_}, curr_{another.curr_} {}
...
public:
    ...
    MyList(std::initializer_list<T> inputList) : MyList(inputList.begin(), inputList.end()) {}
    ~MyList()
    {
        erase(begin(), end());
    }
    ...
    iterator_ begin()
    {
        return iterator_(nullptr, head_);
    }
    iterator_ end()
    {
        return iterator_(tail_, nullptr);
    }
    ...
    //void insert(iterator_ pos, const T &value) - Insertuje vrijednost "value" na mjesto prije iteratora "pos"
